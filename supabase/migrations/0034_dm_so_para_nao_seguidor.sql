-- INCIDENTE 20/08/2026: "Qualificado" filtrava apenas <> 'FOLLOWS', então
-- UNKNOWN (sem prova nenhuma) entrava na campanha em massa — 118 DMs saíram
-- para gente sem status comprovado, parte delas seguidores reais que o export
-- não casou por username. REGRA REFEITA: listas em massa (qualificados,
-- elegíveis, resumo) exigem follow_status = 'NOT_FOLLOWING' COMPROVADO.
-- UNKNOWN sai dessas listas e só recebe DM por aprovação individual.
...
language sql stable as $$
  ...
  where coalesce(p.is_blacklisted, false) = false
    -- SÓ quem comprovadamente NÃO segue. UNKNOWN fora (incidente 20/08).
    and p.follow_status = 'NOT_FOLLOWING'
...
language sql stable as $$
  ...
  where coalesce(u.is_blacklisted, false) = false
    -- QUALIFICADO = comprovadamente não segue. UNKNOWN fora (incidente 20/08).
    and u.follow_status = 'NOT_FOLLOWING'
...
create function oportunidades_resumo()
returns table (
  ...
  removidas_follow_desconhecido bigint,
...
language sql stable as $$
  ...
  julgadas as (
    select p.uid, p.eventos,
           ...
           -- sem prova de que NÃO segue (e também não é FOLLOWS): fora da massa
           coalesce(u.follow_status, 'UNKNOWN') not in ('FOLLOWS', 'NOT_FOLLOWING') desconhecido,
  ...
  select
    ...
    (select count(*) from julgadas where not bl and not segue and not desconhecido and not recente and not na_fila),
    (select coalesce(sum(eventos - 1), 0) from julgadas),
    (select count(*) from julgadas where segue),
    (select count(*) from julgadas where desconhecido and not bl),
    (select count(*) from julgadas where recente and not segue and not desconhecido),
    (select count(*) from julgadas where bl),
    (select count(*) from julgadas where na_fila and not recente and not segue and not desconhecido and not bl);
$$;
